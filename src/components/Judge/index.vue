...
<template>
  <div class="question-list">
    <h3>判断题（共{{ data.length }}题<span v-if="isTest">）</span><span v-if="!isTest">，每小题{{ score }}分）</span></h3>
    <dl
    ...
    >
      ...
      <template v-if="!isTest">
        <dd class="question-list-item-option question-list-item-answer">
          你的答案：{{ map[item.answer] }}
        </dd>
        <dd class="question-list-item-option question-list-item-correctAnswer">
          正确答案：{{ item.original.correctAnswer | trueOrFalseFilter }}
        </dd>
      </template>
